...
function createPasswordResetWorkflow({ config, storage, logger, retryWithBackoff, withTimeout }) {
  if (!storage) {
    throw new Error("Storage client is required for password-reset workflow");
  }

  const workflowLogger = logger.child({ workflow: "password-reset" });
  ...
  async function buildEmail({ template, payload, attachments, correlationId }) {
    ...
    return {
      subject: payload.subject || rendered.subject || "Reset your password",
      html: rendered.html,
      text: rendered.text,
