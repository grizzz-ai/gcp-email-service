...
function createInviteWorkflow({ config, storage, logger, retryWithBackoff, withTimeout }) {
  if (!storage) {
    throw new Error("Storage client is required for invite workflow");
  }

  const workflowLogger = logger.child({ workflow: "invite" });
  ...
  async function buildEmail({ template, payload, attachments, correlationId }) {
    ...
    return {
      subject: payload.subject || rendered.subject || "You've been invited",
      html: rendered.html,
      text: rendered.text,
